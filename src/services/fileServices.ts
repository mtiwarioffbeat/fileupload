...
// export async function saveUserFile(userId: number, file: File, filePath: string) {
//   const client = await pool.connect();
//   try {
//     await client.query("BEGIN");
//     await client.query(
//       `INSERT INTO userfiles (id, filename, filepath, filetype)
//        VALUES ($1, $2, $3, $4)`,
//       [userId, file.name, filePath, file.type]
//     );
//     await client.query("COMMIT");
...
//       `SELECT id, filename, filepath, filetype, created_at
//        FROM userfiles WHERE id = $1 ORDER BY created_at DESC`,
//       [userId]
//     );
//     return res.rows;
//   } finally {
...
//   try {
//     const res = await client.query(
//       `DELETE FROM userfiles WHERE filepath = $1 RETURNING *`,
//       [filepath]
//     );
//     return res.rows[0];
//   } finally {
